...
| `/conversations/all` | `site:configure` | **built** — `26-90`, row tap **built** — `26-98` | A five-column table (visitor, state, assigned operator, started, unread) becomes a card list with a filter chip row. **Shipped as a third segment of the Диалоги list rather than an overflow route** (`navigation.md`'s own decision list): a status filter (Не начат / Назначен / Закрыт, the first two on by default) applied server-side as a repeatable `state` parameter, keyset paging on scroll, and swipe-left erase for a holder of `conversation:erase` — a `202 Accepted` holds the row in a visible «Стирается…» state until the server stops returning it, never an optimistic removal, matching `AdminConversationsPage`'s own `erasedIds` rule. `26-90` shipped the row as deliberately not tappable, on the honest ground that the server had no read for it (`navigation.md`'s own account of what changed and why); `26-98` closed that gap with a genuinely new server read (`GetConversationHistoryAsSiteConfigureHolderQuery`, gated on the identical `site:configure` permission, never assigning), so a row now opens `ThreadScreen` in a read-only mode — no composer, no contact panel, no mark-read — rather than staying inert. |
...
| `/calendar/services` | `calendar:configure` | **as-is** | List + add, price in whole roubles with the optional "от" flag, optional description. **`26-96` closed the gap this row used to name**: `PUT /services/{id}` exists, so a per-row edit is real on both surfaces, and the app shipped its own read-plus-edit Услуги screen as a fourth segment of Записи. There is still **no delete, on purpose** — archiving (`isActive`) is what this product offers instead, because four server-side read models resolve a past booking's service name through the same row (`Ago.Calendar.Domain.Service.IsActive`). Creating a service from the phone is still not built; that stays with whichever item scopes the full screen. |
...
| **Notification settings** | Android notification channels are an OS-level concept with no web counterpart, and push is this app's own first dependency rather than a caveat on it (`plan.md`, "Push is the reason this app exists"). This row originally described a screen designed against all five kinds the mockup drew, every switch disabled behind one banner until push landed. **`26-19` shipped the real thing instead of un-gating that design**: only the two kinds `26-05`'s fan-out actually sends (назначенный диалог, сообщение в открытом диалоге) get a row, each deep-linking into the system's own per-channel settings rather than a disagreeing in-app copy; the other three kinds (запись ждёт решения, дедлайн подтверждения, сообщение команде) are not drawn at all, because nothing emits them — no disabled row lying about a promise. Quiet hours and the away-state note both shipped as designed. Its ancestor is the console's `AlertSettings`; the mapping is not one-to-one, because a browser has no notification channels. |
