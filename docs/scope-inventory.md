...
| `/conversations/all` | `site:configure` | **built** — `26-90` | A five-column table (visitor, state, assigned operator, started, unread) becomes a card list with a filter chip row. One thing must be *visibly* true rather than a dead tap: an admin here gets read-only summary data and still cannot open another operator's thread (`authorization.md`, `5-08`) — so a row is not tappable-into-a-thread, and says why. **Shipped as a third segment of the Диалоги list rather than an overflow route** (`navigation.md`'s own decision list): a status filter (Не начат / Назначен / Закрыт, the first two on by default) applied server-side as a repeatable `state` parameter, keyset paging on scroll, and swipe-left erase for a holder of `conversation:erase` — a `202 Accepted` holds the row in a visible «Стирается…» state until the server stops returning it, never an optimistic removal, matching `AdminConversationsPage`'s own `erasedIds` rule. The "not tappable" line above is the one point where the approved mockup was drawn against an assumption the server does not hold, and it held: see `navigation.md`. |
